...
Could we then create a new `RasterLayer` object from that matrix?
...
We won't do this in the workshop, but you could imagine that you could then use
...
What does the `@data@values` slot look like in our new object now?
...
writeRaster(DEM_WGS_crop, filename="./data/DEM_reproject_crop.tif", format="GTiff", overwrite = T)
```
