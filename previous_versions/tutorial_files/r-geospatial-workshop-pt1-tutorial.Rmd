...
Use the `Google Geocoding API` to get coordinates for place names and addresses
...
So, we won't call Google APIs in this tutorial.

This set up is beyound the scope of this workshop but we want you to know its there if you want to use the Google Geocoder which is excellent.
...
Alternatively, just follow along without trying while we demo!
...
`ggmap` uses the `get_map` function to fetch map tiles to display in your map.
...
- overlay the `sfhomes` data for `2015

- color code the points by `totvalue`
...
#
sfhomes15_map <- ggmap(sf_basemap) +
...
Create a map of the SF homes, BART Stations and Landmarks on top of the basemap.

```{r, results = "hide"}
sfmap_bart_landmarks <- ggmap(sf_basemap) +
  geom_point(data=sfhomes15, aes(x=lon, y=lat))  +
  geom_point(data=bart, aes(x=X,y=Y), col="red", size=3) +
...
library(knitr)
purl("r-geospatial-workshop-pt1-tutorial.Rmd",
     output = "scripts/r-geospatial-workshop-pt1-tutorial.R", documentation = 1)
