...
- Follow along by opening [r-geospatial-workshop-pt1-sp2018.Rmd](https://github.com/dlab-geo/r-geospatial-workshop/blob/master/r-geospatial-workshop-pt1-sp2018.Rmd) in your web browser
...
```
<img src="images/sfhomes_ggplot1.png", width="600px"></img>

## Subset the Data

on 2015 SalesYear and plot by property value (totvalue)

```{r, eval=F}
sfhomes15 <- subset(sfhomes, as.numeric(SalesYear) == 2015)

nrow(sfhomes15) # How many records?
hist(sfhomes15$totvalue) # What is the distribute of total sales values?
...
sfhomes2010_16 <- subset(sfhomes, as.numeric(SalesYear) > 2005)

ggmap(sf_basemap_lite) +
  geom_point(aes(lon, lat, col=totvalue), data = sfhomes2010_16 )  +
...
* Input/export the variety of spatial data file formats

* Data cleaning! Before mapping & analysis comes `geoprocessing`

* CRS tranformations

* Spatial analysis - spatial metrics and relationships

* Data linkages - when data are in same CRS you can `join by location`
...
Let's transform the sfhomes data frame to an `sp` object

What type of object do you think we will make?
...
Are all the columns that were present in the `sfhomes15` now in the SPDF `sfhomes15_sp`?
...
## Plot  `sfboundary` and `sfhomes15_sp` together
```{r}
...
## CRS Definitions & Transformations

All spatial objects should have a CRS defined.
...
All spatial objects should have the same CRS.
...
proj4string(sfhomes15_sp) <- WGS84_CRS
...
```{r, eval=F}
proj4string(sfhomes15_sp) == proj4string(sf_lonlat)
...
plot(sf_lonlat)
points(sfhomes15_sp, col="red")
points(sfhomes15_sp[sfhomes15_sp$price<100,], col="green", pch=19)
...
```
sf_lonlat <- spTransform(sfboundary, CRS(proj4string(sfhomes15_sp)))
```
...
Use a CRS object
```
WGS84_CRS <- CRS(proj4string(sfhomes15_sp))
sf_lonlat <- spTransform(sfboundary, WGS84_CRS)
```
...
plot(sfhomes15_sp, col="red", bg="lightblue", pch=21, cex=2)
...
## Examples
```
spplot(sfhomes15_sp,"bathrooms")

spplot(sfhomes15_sp,"accommodates")

spplot(sfhomes15_sp,"property_type")

spplot(sfhomes15_sp,"neighbourhood")
...
lines(sf_streets)
points(sfhomes15_sp, col="red")
points(sfhomes15_sp[sfhomes15_sp$price<200,], col="green")
```
...
Make a graduted color map of SF Airbnb rentals (sfhomes15_sp)
...
```{r, eval=F}
library(leaflet)
leaflet(sfhomes15_sp) %>% addTiles() %>%
    addCircleMarkers(data = sfhomes15_sp, radius = 5, stroke=F,
...
```{r, eval=F}
popup_content <- sfhomes15_sp$name
popup_content <- paste0(popup_content, "<br>Price per night: $", sfhomes15_sp$price)
popup_content <- paste0(popup_content, "<br><a href=",sfhomes15_sp$listing_url,">More info...</a>")
...
sfhomes15_sp$coit_dist <- 
  spDistsN1(sfhomes15_sp,c(-122.405837,37.802032), longlat = T) 
